            ...
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  @click="
                    changeAttrActive(attrvalue, attr.spuSaleAttrValueList)
                  "
                >
...
export default {
  ...
  mounted() {
    this.$store.dispatch("detail/getGoodDetailInfo", this.$route.params.skuid);
  },
  computed: {
  ...
  methods: {
    ...
    changeNum(e) {
      ...
      let originalNum = this.skuNum;
      if (num < 1 || isNaN(num)) {
        this.skuNum = originalNum;
        this.$nextTick(() => {
          this.$refs.input.value = originalNum;
        });
        return;
      }
    ...
    async addToShopCart() {
      try{
        ...
        // 浏览器存储复杂数据，主要是用于前端取用数据进行展示的方便，并非给服务器传输方便的考虑
